...
public class HtmlFetcher {
	...
	public static int getStatusCode(Map<String, List<String>> headers) {
		...
		String toParse = list.get(0).substring(9, 12);
	...
	public static String fetch(URL url, int redirects) {
		...
		try (
				...
				List<String> responses = response.lines().toList();
				
				if(getStatusCode(headers) == 200 && isHtml(headers)) {
					html = String.join("\n", responses);
					//System.out.println("If block html: " + html);
				}
				if(isRedirect(headers)) {
					if(redirects > 0) {
						html = fetch(new URL(headers.get("Location").get(0)), redirects - 1);
					}
				}
		}
		catch (IOException e) {
